...
window.addEventListener('load', () => {
    ...
    Swal.fire({
        title: 'Müzik eşliğinde izlemek ister misin? 🎵',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Evet, lütfen!',
        cancelButtonText: 'Hayır, sessiz kalsın',
    }).then((result) => {
        if (result.isConfirmed) {
            // Kullanıcı onayladığı için mute'u kaldırıyoruz ve çalıyoruz
            song.muted = false;
            song.play().catch(e => console.log("Müzik başlatılamadı:", e));
            animationTimeline();
        } else {
            // Kullanıcı hayır derse sessiz devam eder
            animationTimeline();
        }
    });
});
...
const animationTimeline = () => {
    // Balonları otomatik oluşturma (HTML'i temiz tutmak için buraya ekledik)
    const balloonContainer = document.querySelector('.baloons'); // HTML'deki class ismiyle eşleşmeli
    if (balloonContainer) {
        const balloonTypes = ['ballon1.svg', 'ballon2.svg', 'ballon3.svg'];
        for (let i = 0; i < 30; i++) {
            const img = document.createElement('img');
            const randomType = balloonTypes[Math.floor(Math.random() * balloonTypes.length)];
            img.src = `img/${randomType}`;
            balloonContainer.appendChild(img);
        }
    }

    // Karakterleri ayırma işlemi
